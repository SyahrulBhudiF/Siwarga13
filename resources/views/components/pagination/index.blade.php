...
@foreach ($elements as $element)
    ...
    @if (is_array($element))
        ...
        @if ($paginator->currentPage() > 1)
            <a
                class="py-2 px-4 text-Neutral/70 rounded-[0.5rem] border border-Neutral/30 text-base transition ease-in-out duration-200"
        ...
        @foreach ($element as $page => $url)
            ...
            @elseif ($page > $paginator->currentPage() && $page <= $end)
                <a class="page-link py-2 px-4 text-Neutral/70 rounded-[0.5rem] border border-Neutral/30 text-base transition ease-in-out duration-200"
                   href="{{ $url }}">{{ $page }}</a>
            @endif
        @endforeach
